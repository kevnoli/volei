...
@router.get('/{group_id}')
def fetch_group(group_id: int, session: Session = Depends(get_session)) -> GroupRead:
    return group.index(group_id, session)


@router.post('', status_code=201)
def create_group(group_data: GroupCreate, session: Session = Depends(get_session)) -> GroupRead:
...
@router.put('/{group_id}')
def replace_group(group_id: int, group_data: GroupUpdate, session: Session = Depends(get_session)) -> GroupRead:
